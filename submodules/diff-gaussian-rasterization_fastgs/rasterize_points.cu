	...
	const bool get_flag)
{
  ...
  return std::make_tuple(rendered, num_buckets, out_color, radii, geomBuffer, binningBuffer, imgBuffer, sampleBuffer, metricCount, out_depth, out_alpha);
}
	...
	const bool debug) 
{
  ...
  // Per-Gaussian gradient of the loss w.r.t. its view-space depth. Internal:
  // BACKWARD::preprocess folds it straight into dL_dmeans3D.
